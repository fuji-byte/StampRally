...
export default function AdminStampPage() {
  ...
  const handleSort = () => {
    const sortedData = data.filter((user) => {
      return [1, 2, 3, 4, 5].every(
        (i) => user[`stamp${i}` as keyof StampWithEmail] === true,
      );
    });
    setData(sortedData);
  };
  ...
  return errorMessage ? (
    ...
    <div className="p-6">
      <button
        onClick={() => router.push("/admin/rand")}
        className="mb-4 bg-gray-200 text-black hover:bg-gray-300 px-4 py-2 rounded border border-gray-300"
      >
        抽選管理画面に移動
      </button>
      ...
      <div className="flex gap-5 pb-5">
        <button
          onClick={handleReset}
          className="px-5 py-2 bg-white text-black rounded border-2 border-gray-soft hover:bg-gray-400"
        >
          リセット
        </button>
        <button
          onClick={handleSort}
          className="px-5 py-2 bg-blue-500 text-white rounded border-2 border-gray-soft hover:bg-blue-600"
        >
          全所持ソート
        </button>
      </div>
      <div className="flex gap-3 pb-5">
        ...
        <button
          ...
          className="px-3 py-1 bg-white text-black rounded border-2 border-gray-soft hover:bg-gray-400"
        >
          検索
        </button>
      </div>
      <table className="table-auto border w-full text-sm">
        <thead>
          <tr>
            {[
              <th key="email" className="border px-2 py-1">
                Email
              </th>,
              <th key="userId" className="border px-2 py-1">
                UserId
              </th>,
              ...[1, 2, 3, 4, 5].map((i) => (
              ...
              ...[1, 2, 3, 4, 5].map((i) => (
                <th key={`i${i}`} className="border px-2 py-1">
                  Inner{i}
                </th>
              )),
            ]}
